...
const UserData = ({ onClick, showModal, count, user }) => {
  return (
      ...
      {Object.keys(user).length > 0 ? (
        <>
          <div className="flex cursor-pointer items-center gap-2" onClick={onClick}>
            ...
            />
            <p className="text-base text-slate-700">{user?.name ? shortifyName(user.name) : ''}</p>
          </div>
          <div
          ...
          >
            ...
            <Gap height={10} />
            <Button small type="submit" className="font-medium text-slate-700">
              logout
            </Button>
          </div>
        </>
